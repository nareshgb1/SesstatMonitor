...
from
	...
	where stat_name in ('session logical reads', 'physical reads', 'physical reads direct', 'consistent changes',
	  ...
	  and h.snap_id = s.snap_id and begin_interval_time between  trunc(sysdate-30) and sysdate + 1
   		and h.instance_number = s.instance_number) e,
	v$database
where value > 0
...
select dbname, sday, dow, weekno, 
	...
	physical_reads-prds_direct prds_cache, prds_temp , pwts_temp, prds_lob, pwts_lob
from h order by 1, 2
/
